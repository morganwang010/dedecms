		...
		if(pagination==true) {
			...
			for (li = 0; li < amountSlide; li++){
				...
				if(opts.thumbs==true) {
					...
					var newImg = $("<span/>");
					newImg.html(dataThumb);
					$('li#pag_nav_'+li).append(newImg.attr('class','pix_thumb').css({position:'absolute',color:'#ffffff'}).animate({opacity:0},0));
					newImg.css('text-indent','1px');
					$('li#pag_nav_'+li+' > span').after('<div class="thumb_arrow" />');
					$('li#pag_nav_'+li+' > .thumb_arrow').animate({opacity:0},0);
					
					if(!isMobile()){
						$('#pix_pag_ul li').find('span:first').hover(function(){
							var cobject = $(this).closest('li');
							$('.pix_thumb, .thumb_arrow',cobject).addClass('visible').stop(true,false).animate({'margin-bottom':20, opacity: 1},300,'easeOutQuad');					
						},function(){
								var cobject = $(this).closest('li');
							$('.pix_thumb, .thumb_arrow',cobject).removeClass('visible').stop(true,false).animate({'margin-top':0, opacity: 0},150);					
						});
					}
				}
			}
